...
use resdb::{
	model::{OrderPayment, UserRole},
	order_payment::db::OrderPaymentExt,
};
use serde_json::json;
...
async fn get_payments(
	...
	let query_result =
		app_state.db_client.get_order_payments(page, limit).await;

	if query_result.is_err() {
		let message =
			"Something bad happened while fetching all order payments items";
...
async fn update(
	...
	let data = body.into_inner();
	let query_result =
		app_state.db_client.order_payment_update(pid, data).await;
...
async fn delete(
	...
	match query_result {
		Ok(rows_affected) => {
			if rows_affected == 0 {
				let message =
					format!("Order payment with ID: {} not found", pid);

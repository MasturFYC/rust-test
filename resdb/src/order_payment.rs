pub mod db {
    ...
    pub trait OrderPaymentExt {
        async fn get_order_payment(&self, id: Uuid) -> Result<Option<OrderPayments>, Error>;
        async fn get_order_payments(
            &self,
            page: usize,
            limit: usize,
        ) -> Result<(Vec<OrderPayments>, i64), Error>;
        async fn order_payment_create<T>(&self, data: T) -> Result<Option<OrderPayments>, Error>
        where
    ...
    impl OrderPaymentExt for DBClient {
            ...
            T: Into<OrderPayment> + Send,
        {
            ...
            let (ledger_details, count) = LedgerUtil::from_order_payment(&op.amount, pid, pid);

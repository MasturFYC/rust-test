...
pub mod db {
    ...
    impl CategoryExt for DBClient {
            ...
            T: Into<Category> + Send,
        {
            ...
            let query_result =
                sqlx::query_file_as!(Categories, "sql/category-insert.sql", cat.name)
                    .fetch_optional(&self.pool)
                    .await?;
            ...
            T: Into<Category> + Send,
        {
            let cat: Category = data.try_into().unwrap();
            let query_result =
                sqlx::query_file_as!(Categories, "sql/category-update.sql", id, cat.name)
                    .fetch_optional(&self.pool)
                    .await?;
        ...
        async fn get_category_with_products(
            ...
            let query_result =
                sqlx::query_file_as!(CategoryWithProduct, "sql/category-with-product.sql", id)
                    .fetch_optional(&self.pool)
                    .await?;
            Ok(query_result)
        }

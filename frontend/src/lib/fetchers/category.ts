...
export async function getCategoryProp(): Promise<iPropertyWithID> {
  ...
  const request = new Request(`${baseURL}/categories/property/list`, options);
...
export async function getRelationProp(p: string[]): Promise<iRelationResult> {
...
export async function getBarcodes(): Promise<{
  status: string;
  data: { barcode: string }[];
}> {

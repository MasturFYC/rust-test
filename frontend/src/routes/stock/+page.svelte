<script lang="ts">
  ...
  import { getStocks, postCreateStock } from "./handler";
  ...
  function editStock(e: CustomEvent<number>): void {
    if (e.detail === 0) {
      stock = { ...initData };
      details = [];
      isEdit = true;
...
{#if $supplierQuery.isLoading || $employeeQuery.isLoading || $queryBarcode.isLoading || $queryStock.isLoading}
...
{:else}
  ...
  />
  <Pagination
    totalItems={$queryStock.data?.totalItems}

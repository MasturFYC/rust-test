<script lang="ts">
  ...
  import { initStock, stock, details } from "./store";
  ...
  function editStock(e: CustomEvent<number>) {
    changeStockSession(e.detail);
    isEdit = true;
  }

  async function changeStockSession(id: number) {
    stockId = id;
    await tick();
  ...
  async function saveStock(e: CustomEvent<iStockDetail[]>) {
    if (e.detail.length > 0) {
      ...
      const savedDetails: iStockDetail[] = $details.map((m) => ({
        ...m,
        stockId: $stock.id,
      }));
      ...
      if ($stock.id === 0) {
        const result = await postCreateStock(savedStock, savedDetails);
        if (result) {
        ...
        const result = await postUpdateStock(stockId, savedStock, savedDetails);
        if (result) {
          await client.invalidateQueries([qKey, { id: stockId }]);
          await client.invalidateQueries([
            qKey,

...
import { baseURL, credential_include, type iGudang, type iStockDetail } from '$lib/interfaces';
import {
	Button,
	DataTable,
	TextInput,
...
function onRowClick(e: CustomEvent<DataTableRow>) {
	...
	if (i < 0) {
		details.update((o) => [...o, { ...initDetail, gudangName: getDefaultGudangName() }]);
	}
...
async function onTableKeyDown(e: KeyboardEvent & { currentTarget: EventTarget & HTMLDivElement }) {
	// console.log(e.key)
	if (
		e.key !== '+' &&
		e.key !== '=' &&
...
async function barcodeOnChange(e: CustomEvent<string | number | null>, id: number) {
...
<div>
	<Button
		...
		disabled={$isStockUpdating}>Total: {items} item{items > 1 ? 's' : ''}</Button
	>
</div>
